...
Codex Mixin 已具备本地网关、菜单栏 app、launchd 常驻、登录时启动并开启服务、自动检查更新、供应商预设、模型 catalog 生成、上下文 metadata 补齐、Codex 配置安装/恢复、额度显示和截图中的设置界面。

后续更适合继续补的是打包签名和 notarization、更多供应商预设，以及针对团队分发的配置模板。
