...
func localizedPrompt(_ text: String) -> String {
    let translations: [String: (traditional: String, english: String)] = [
        ...
        "配置备份已导出": ("設定備份已匯出", "Configuration Backup Exported"),
        "导出配置备份失败": ("匯出設定備份失敗", "Unable to Export Configuration Backup"),
        "导入配置备份？": ("匯入設定備份？", "Import Configuration Backup?"),
        "配置备份已导入": ("設定備份已匯入", "Configuration Backup Imported"),
        "配置已导入，但应用失败": ("設定已匯入，但套用失敗", "Configuration Imported but Not Applied"),
        "导入配置备份失败": ("匯入設定備份失敗", "Unable to Import Configuration Backup"),
        ...
        "安装到 Pi 失败": ("安裝到 Pi 失敗", "Unable to Install into Pi"),
        "从 Pi 卸载失败": ("從 Pi 解除安裝失敗", "Unable to Remove Pi Integration"),
        "Pi 配置已恢复": ("Pi 設定已還原", "Pi Configuration Restored"),
