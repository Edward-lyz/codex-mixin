...
启用 Baidu OneAPI 的「DUCX app-server」后，每一条 Responses 上游子请求都会先创建一个
真实的 DUCX turn。DUCX 把它认证的 HTTP 请求发到仅监听 `127.0.0.1` 的本机桥；桥使用
一次性 request id 绑定原始请求，移除 DUCX 注入的提示词、工具和输入后，再把 DUCX
生成的请求头原样转发给 OneAPI。Codex Mixin 不提取、缓存、重建或伪造认证 Header，
一次性 request id 也不能重放。Fusion、Web Search、画图和 Auto Review 产生的 Responses
子请求都走同一个统一执行层，不会绕过 DUCX。

网关启动时不会启动 DUCX；首次请求初始化完成后，后续请求复用同一进程，只承担一次
本机回环转发，不会在热路径反复拉起子进程。初始化会禁用已知的 DUCX hooks、插件、
内置工具和技能，并强制检查 DUCX 暴露的 hook 数量为零；检查失败就拒绝转发。DUCX
仍需读取自身的登录信息与模型代理配置，但 Codex Mixin 不会写入这些配置。仓库中的
隔离探针会对 `config.toml`、`hooks.json` 和报告相关文件做前后指纹检查。
...
When “DUCX app-server” is enabled for Baidu OneAPI, every upstream Responses subrequest first
creates a real DUCX turn. DUCX sends its authenticated HTTP request to a loopback-only bridge.
The bridge binds that request to the caller payload with a single-use request ID, removes DUCX
prompt/tool/input additions, and relays the headers generated by DUCX unchanged to OneAPI.
Codex Mixin does not extract, cache, reconstruct, or forge authentication headers, and request IDs
cannot be replayed. Responses subrequests created by Fusion, Web Search, image generation, and
Auto Review use the same executor and cannot bypass DUCX.

DUCX starts lazily and remains alive for reuse, so the hot path adds one loopback relay without
spawning a process per request. Initialization disables known DUCX hooks, plugins, built-in tools,
and skills, and fails closed unless DUCX reports zero exposed hooks. DUCX still reads its own login
and model-proxy configuration, but Codex Mixin never writes those files. The isolated repository
probe fingerprints `config.toml`, `hooks.json`, and reporting-related files before and after
execution. `scripts/verify_ducx_oneapi_transport.mjs` points real DUCX at a local mock OneAPI and
verifies its authentication header and multimodal input. Even if DUCX declares extra tools, the
bridge replaces the complete DUCX request body with the caller payload before it reaches OneAPI.
...
cannot be overridden, and `comate_custom_header` is reserved exclusively for the current DUCX
