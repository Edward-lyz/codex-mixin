...
- Fusion 多模型编排：多个 Panel 并行分析，经 Judge 汇总后由 Final 模型流式回答；可选择是否在回答中展示中间结果。
...
- Linux x86_64: `codex-mixin-cli-<version>-x86_64-unknown-linux-gnu.tar.gz` 或 `codex-mixin-<version>-x86_64-unknown-linux-gnu.deb`
- Linux ARM64: `codex-mixin-cli-<version>-aarch64-unknown-linux-gnu.tar.gz` 或 `codex-mixin-<version>-aarch64-unknown-linux-gnu.deb`
...
高级选项 `在回答中显示 Panel / Judge 中间结果` 默认开启，对应 stored config 中的 `show_intermediate_results: true`：

1. `Fusion · Panel Results`：所有 Panel 按配置顺序放在一张表中，每个输出通过 `<details>` 折叠。
2. `Fusion · Judge Result`：显示 Judge 模型、状态和汇总内容。
3. `Fusion · Final Answer`：显示 Final 模型标题，随后流式输出最终回答。
...
- Orchestrates multiple Panel models in parallel, compares them with a Judge model, and streams a Final answer, with optional intermediate-result visibility.
...
- macOS Apple Silicon: `codex-mixin-cli-<version>-aarch64-apple-darwin.tar.gz`
- macOS Intel: `codex-mixin-cli-<version>-x86_64-apple-darwin.tar.gz`
- Linux x86_64: `codex-mixin-cli-<version>-x86_64-unknown-linux-gnu.tar.gz` or `codex-mixin-<version>-x86_64-unknown-linux-gnu.deb`
- Linux ARM64: `codex-mixin-cli-<version>-aarch64-unknown-linux-gnu.tar.gz` or `codex-mixin-<version>-aarch64-unknown-linux-gnu.deb`
...
`Show Panel / Judge intermediate results` is enabled by default and maps to `show_intermediate_results: true` in stored config. When enabled, Codex shows:

1. `Fusion · Panel Results`: one table in configured Panel order, with each result collapsed under `<details>`.
2. `Fusion · Judge Result`: the Judge model, status, and synthesis.
3. `Fusion · Final Answer`: the Final model heading followed by its streamed answer.
