...
- 常驻服务：后台 daemon 在退出菜单栏 App 后仍可运行；启用登录自启时改由 launchd 托管。
...
| Mac 架构 | 下载文件 |
| --- | --- |
| Apple Silicon | `codex-mixin-0.2.8-aarch64-apple-darwin.dmg` |
| Intel | `codex-mixin-0.2.8-x86_64-apple-darwin.dmg` |
...
```bash
xattr -dr com.apple.quarantine codex-mixin-0.2.8-aarch64-apple-darwin.dmg
...
- macOS Apple Silicon: `codex-mixin-cli-0.2.8-aarch64-apple-darwin.tar.gz`
- macOS Intel: `codex-mixin-cli-0.2.8-x86_64-apple-darwin.tar.gz`
- Linux x86_64: `codex-mixin-cli-0.2.8-x86_64-unknown-linux-gnu.tar.gz` 或 `codex-mixin-0.2.8-x86_64-unknown-linux-gnu.deb`
- Linux ARM64: `codex-mixin-cli-0.2.8-aarch64-unknown-linux-gnu.tar.gz` 或 `codex-mixin-0.2.8-aarch64-unknown-linux-gnu.deb`
...
打开 App 会自动启动本地网关。关闭终端或退出菜单栏 App 后，后台网关仍可以继续运行；只有打开 `登录时启动并开启服务` 时才会安装 launchd agent。需要临时停服务时使用菜单里的暂停动作。
...
- Keeps the background daemon running after the app exits, with launchd reserved for explicit login startup.
...
- macOS Apple Silicon: `codex-mixin-cli-0.2.8-aarch64-apple-darwin.tar.gz`
- macOS Intel: `codex-mixin-cli-0.2.8-x86_64-apple-darwin.tar.gz`
- Linux x86_64: `codex-mixin-cli-0.2.8-x86_64-unknown-linux-gnu.tar.gz` or `codex-mixin-0.2.8-x86_64-unknown-linux-gnu.deb`
- Linux ARM64: `codex-mixin-cli-0.2.8-aarch64-unknown-linux-gnu.tar.gz` or `codex-mixin-0.2.8-aarch64-unknown-linux-gnu.deb`
