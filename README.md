...
| Mac 架构 | 下载文件 |
| --- | --- |
| Apple Silicon | `codex-mixin-0.2.3-aarch64-apple-darwin.dmg` |
| Intel | `codex-mixin-0.2.3-x86_64-apple-darwin.dmg` |
...
```bash
xattr -dr com.apple.quarantine codex-mixin-0.2.3-aarch64-apple-darwin.dmg
...
- macOS Apple Silicon: `codex-mixin-cli-0.2.3-aarch64-apple-darwin.tar.gz`
- macOS Intel: `codex-mixin-cli-0.2.3-x86_64-apple-darwin.tar.gz`
- Linux x86_64: `codex-mixin-cli-0.2.3-x86_64-unknown-linux-gnu.tar.gz` 或 `codex-mixin-0.2.3-x86_64-unknown-linux-gnu.deb`
- Linux ARM64: `codex-mixin-cli-0.2.3-aarch64-unknown-linux-gnu.tar.gz` 或 `codex-mixin-0.2.3-aarch64-unknown-linux-gnu.deb`
...
4. 注册独立的 `custom` provider，不覆盖 Codex 内置 `openai` provider。
5. 将顶层 `model_provider` 设置为 `custom`，由本地网关分流官方 GPT 与自定义模型。
6. 标记 `custom` provider 仍然使用 Codex 官方 OAuth 能力。
...
最新版 Codex 禁止覆盖内置 `openai` provider，因此 Codex Mixin 始终使用独立的 `custom` provider。官方 GPT 请求仍由网关转发到官方 Codex backend，自定义模型请求转发到已配置的上游。
...
- macOS Apple Silicon: `codex-mixin-cli-0.2.3-aarch64-apple-darwin.tar.gz`
- macOS Intel: `codex-mixin-cli-0.2.3-x86_64-apple-darwin.tar.gz`
- Linux x86_64: `codex-mixin-cli-0.2.3-x86_64-unknown-linux-gnu.tar.gz` or `codex-mixin-0.2.3-x86_64-unknown-linux-gnu.deb`
- Linux ARM64: `codex-mixin-cli-0.2.3-aarch64-unknown-linux-gnu.tar.gz` or `codex-mixin-0.2.3-aarch64-unknown-linux-gnu.deb`
...
4. Registers a separate `custom` provider without overriding Codex's built-in `openai` provider.
5. Sets `model_provider = "custom"`; the local gateway routes official GPT and custom models separately.
6. Marks the `custom` provider as OpenAI-authenticated and websocket-capable.
