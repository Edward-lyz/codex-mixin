...
#[tokio::test]
async fn skips_anthropic_web_search_without_query() {
    let events = [
        json!({"type":"message_start","message":{"usage":{"input_tokens":10}}}),
        json!({"type":"content_block_start","index":1,"content_block":{"type":"server_tool_use","id":"srvtoolu_empty","name":"web_search","input":{}}}),
        json!({"type":"content_block_delta","index":1,"delta":{"type":"input_json_delta","partial_json":"{}"}}),
        json!({"type":"content_block_stop","index":1}),
        json!({"type":"content_block_start","index":2,"content_block":{"type":"web_search_tool_result","tool_use_id":"srvtoolu_empty","content":[]}}),
        json!({"type":"content_block_stop","index":2}),
        json!({"type":"content_block_start","index":3,"content_block":{"type":"text"}}),
        json!({"type":"content_block_delta","index":3,"delta":{"type":"text_delta","text":"Search skipped."}}),
    ...
    let body = map_anthropic_events(&events).await;
    assert!(!body.contains("response.failed"), "{body}");
    assert!(!body.contains("web_search_call"), "{body}");
    assert!(body.contains("Search skipped."), "{body}");
