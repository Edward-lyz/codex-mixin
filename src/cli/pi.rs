...
fn gateway_credential(config: &GatewayConfig) -> anyhow::Result<String> {
    let key = config
        .gateway_client_keys
        .get(codex_mixin::gateway_access::GatewayClient::Pi)
        .ok_or_else(|| anyhow::anyhow!("Pi client key is missing"))?;
    anyhow::ensure!(!key.trim().is_empty(), "Pi client key must not be empty");
    anyhow::ensure!(key == key.trim(), "Pi client key has whitespace");
    Ok(key.to_owned())
}
