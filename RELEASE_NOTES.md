...
本版本扩展 Claude Code 和 OpenCode 集成，补充 Amazon Bedrock 支持，并改进模型管理、图片生成、macOS 更新和 gateway 稳定性。

### 应用与协议兼容

- Claude Code 支持分别映射 Opus、Sonnet 和 Haiku 模型，并可从 macOS、TUI 或 CLI 安装和恢复配置
- 新增 OpenCode 集成，在保留现有配置的同时接入已选模型、Fusion 模型和 reasoning variants；gateway 凭据单独保存在权限受限的文件中
...
- 增加 Anthropic 协议转换、OpenCode 配置、Claude Code 模型映射、官方模型刷新、macOS 更新和 gateway 重启相关回归测试
...
本版本擴充 Claude Code 和 OpenCode 整合，補充 Amazon Bedrock 支援，並改善模型管理、圖片生成、macOS 更新和 gateway 穩定性。

### 應用程式與協定相容性

- Claude Code 支援分別映射 Opus、Sonnet 和 Haiku 模型，並可從 macOS、TUI 或 CLI 安裝和還原設定
- 新增 OpenCode 整合，在保留現有設定的同時接入已選模型、Fusion 模型和 reasoning variants；gateway 憑證單獨儲存在權限受限的檔案中
...
- 增加 Anthropic 協定轉換、OpenCode 設定、Claude Code 模型映射、官方模型更新、macOS 更新和 gateway 重新啟動相關回歸測試
...
This release expands Claude Code and OpenCode integration, adds Amazon Bedrock support, and improves model management, image generation, macOS updates, and gateway reliability.
...
- Add OpenCode integration while preserving existing configuration, exposing selected and Fusion models with reasoning variants, and storing gateway credentials in a separate owner-only file
...
- Add regression coverage for Anthropic protocol translation, OpenCode configuration, Claude Code model mapping, official model refresh, macOS updates, and gateway restarts
