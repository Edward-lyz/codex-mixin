...
- 自定义模型支持本地确定性 `compact`，保留最近对话文本并生成加密的本地 continuation checkpoint
- 官方模型目录安装与刷新流程同步维护本地缓存，并保留用户的模型选择

### 性能优化

- `models.dev` 目录改为跨 Provider 共享进程内缓存，并使用本地持久化缓存与 `ETag` / `Last-Modified` 条件请求
- 远端目录最多每 3 小时检查一次，未变化时不下载完整目录；刷新失败时复用旧缓存并延迟重试
- 自定义模型 `compact` 不再调用上游模型生成摘要，减少网络请求、等待时间和 token 消耗

### BUG 修复

- 修复上游确定性 4xx 被改写为通用 502 的问题，保留原始 status、`Content-Type` 和错误正文
- 修复 DeepSeek 等 Provider 重放缺少 `call_id` 的 named `function_call_output` 导致请求失败的问题
- 修复 Anthropic 工具 schema 顶层 `oneOf`、`anyOf`、`allOf` 导致请求被拒的问题
- 修复官方模型重放其他 Provider 的 reasoning history 导致请求失败的问题
- 修复超过 16 MiB 的 Responses WebSocket 请求被拒绝的问题
- 修复 DSH 0.1.5+ 凭据格式迁移、运行中密钥刷新，以及 macOS gateway 启动等待过短的问题
- 修复 OpenCode reporting plugin 在新版 Bun 中因 stdin 参数格式错误导致 prompt 提交失败的问题
- 修复 macOS 26 下 Provider picker 宽度、ImageGen 端口和测速完成后进度条显示问题
...
- 修复「模型与服务」测速结束后进度条仍停留在界面的问题，只在任务运行期间显示进度
...
- 自訂模型支援本機確定性 `compact`，保留最近對話文字並產生加密的本機 continuation checkpoint
- 官方模型目錄安裝與重新整理流程同步維護本機快取，並保留使用者的模型選擇

### 效能優化

- `models.dev` 目錄改為跨 Provider 共用程序內快取，並使用本機持久化快取與 `ETag` / `Last-Modified` 條件請求
- 遠端目錄最多每 3 小時檢查一次，未變更時不下載完整目錄；重新整理失敗時重用舊快取並延遲重試
- 自訂模型 `compact` 不再呼叫上游模型產生摘要，減少網路請求、等待時間和 token 消耗

### BUG 修正

- 修正上游確定性 4xx 被改寫為通用 502 的問題，保留原始 status、`Content-Type` 和錯誤正文
- 修正 DeepSeek 等 Provider 重播缺少 `call_id` 的 named `function_call_output` 導致請求失敗的問題
- 修正 Anthropic 工具 schema 頂層 `oneOf`、`anyOf`、`allOf` 導致請求被拒的問題
- 修正官方模型重播其他 Provider 的 reasoning history 導致請求失敗的問題
- 修正超過 16 MiB 的 Responses WebSocket 請求被拒絕的問題
- 修正 DSH 0.1.5+ 憑據格式遷移、執行中金鑰重新整理，以及 macOS gateway 啟動等待過短的問題
- 修正 OpenCode reporting plugin 在新版 Bun 中因 stdin 參數格式錯誤導致 prompt 提交失敗的問題
- 修正 macOS 26 下 Provider picker 寬度、ImageGen 連接埠和測速完成後進度列顯示問題
...
- 修正「模型與服務」測速結束後進度列仍停留在介面的問題，只在任務執行期間顯示進度
...
- Add deterministic local compaction for custom models, retaining recent conversation text in an encrypted local continuation checkpoint
- Keep the local official-model cache synchronized during installation and refresh while preserving the user's model selection

### Performance

- Share the `models.dev` catalog across Providers in-process and use a persistent local cache with `ETag` / `Last-Modified` conditional requests
- Check the remote catalog at most every three hours, avoid downloading unchanged content, and reuse stale data with delayed retry after refresh failures
- Stop calling an upstream model to summarize custom-model compaction, reducing network requests, latency, and token usage

### Bug fixes

- Preserve the original status, `Content-Type`, and body for deterministic upstream 4xx responses instead of rewriting them as a generic 502
- Fix DeepSeek and other Provider requests that replay named `function_call_output` items without a `call_id`
- Fix Anthropic requests rejected when tool schemas use root-level `oneOf`, `anyOf`, or `allOf`
- Fix official-model replay of reasoning history generated by another Provider
- Accept Responses WebSocket requests larger than 16 MiB
- Fix DSH 0.1.5+ credential migration and live key refresh, and extend the macOS gateway startup wait
- Fix OpenCode prompt submission failures on newer Bun versions by piping reporting-plugin stdin correctly
- Fix macOS 26 Provider picker sizing, ImageGen port selection, and benchmark progress remaining visible after completion
...
- Hide the Models and Services benchmark progress bar when its run finishes instead of leaving a completed bar on screen
...
- 支持通过自定义模型选择器把已选 Provider 模型直接安装到 Claude Code
...
- 支援透過自訂模型選擇器把已選 Provider 模型直接安裝到 Claude Code
...
- Install selected Provider models directly into Claude Code through its custom model picker
